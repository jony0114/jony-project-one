...
while1("bh,sj","fcw_jia_jc where bh='".$bh."' and uid='".$_SESSION[FCWuser]."'");if(!$row1=mysql_fetch_array($res1)){exit;}
$userid=returnuserid($_SESSION[FCWuser]);
$tplj1="upload/".$userid."/jia/".$bh."/";

 if($_GET[action]=="update"){  //上传
 $sj=date("Y-m-d H:i:s");
 $nbh=time();
 $wjv=$tplj1.$nbh.".jpg";
 intotable("fcw_clear","bh,tp,type1,sj","'".$bh."','".$wjv."','家居建材','".$sj."'");$clearid=mysql_insert_id();
 uploadtpnodata(1,$tplj1,$nbh.".jpg","allpic",800,800,300,300);
 php_toheader("jcupload.php?bh=".$bh."&tpbh=".$nbh."&clearid=".$clearid);
 
 }elseif($_GET[action]=="del"){ //删除
  if(panduan("uid,bh","fcw_jia_jc where uid='".$_SESSION[FCWuser]."' and bh='".$_GET[bh]."'")==1){
  ...
  php_toheader("jcupload.php?bh=".$_GET[bh]);
  }
 }
...
document.getElementById("uping").style.display="";
location.href="jcupload.php?tpbh=<?=$_GET[tpbh]?>&action=del&bh=<?=$_GET[bh]?>&tpid=<?=$_GET[tpid]?>&clearid=<?=$_GET[clearid]?>";	
}
</script>
</head>
<body>
...
<!--等待上传开始-->
<form method="post" name="tpf" enctype="multipart/form-data" action="jcupload.php?tpbh=<?=$_GET[tpbh]?>&action=update&bh=<?=$_GET[bh]?>">
