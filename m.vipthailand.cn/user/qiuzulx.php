...
if($_GET[control]=="add"){
 zwzr();
 while1("id,nc,mot,uid,zjuid","fcw_user where uid='".$_SESSION[FCWuser]."' and (usertype=1 or usertype=2)");if(!$row1=mysql_fetch_array($res1)){Audit_alert("会员验证有误！","qiuzulx.php");}
 $bh=time()."f".$row1[id];
 $sj=date("Y-m-d H:i:s");
 $uip=$_SERVER["REMOTE_ADDR"];
 intotable("fcw_fang","bh,uid,sj,uip,type1,fbty,djl,wylx,ifok,ifxj,zt,mytj,zjuid,zdxh","'".$bh."','".$_SESSION[FCWuser]."','".$sj."','".$uip."','求租',".$_SESSION[FCWuserID].",1,'".sqlzhuru($_POST[R1])."',0,0,99,0,'".$row1[zjuid]."',0");
 php_toheader("qiuzu.php?bh=".$bh);

}elseif($_GET[control]=="update"){
 zwzr();
 updatetable("fcw_fang","wylx='".sqlzhuru($_POST[R1])."' where uid='".$_SESSION[FCWuser]."' and type1='求租' and bh='".$_GET[bh]."'");
...
<div class="right">
 ...
 while0("id,bh,uid,type1,wylx","fcw_fang where uid='".$_SESSION[FCWuser]."' and id=".$_GET[id]." and type1='求租'");if(!$row=mysql_fetch_array($res)){php_toheader("qiuzulist.php");}
 ?>
 <script language="javascript">
 function tj(){
 tjwait();
 ...
 <ul class="uk">
 <li class="l3"></li>
 <li class="l4"><? tjbtnr("下一步","qiuzu.php?bh=".$row[bh])?></li>
 </ul>
 </form>
