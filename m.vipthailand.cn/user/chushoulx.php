...
if($_GET[control]=="add"){
 zwzr();
 while1("id,nc,mot,uid,zjuid,userdj","fcw_user where uid='".$_SESSION[FCWuser]."' and (usertype=1 or usertype=2)");if(!$row1=mysql_fetch_array($res1)){Audit_alert("会员验证有误！","chushoulx.php");}
 $sj=date("Y-m-d H:i:s");
 //判断发布是否超数B
 $djfb=0;
 ...
 intotable("fcw_fang","bh,uid,fbsj,sj,uip,type1,fbty,djl,wylx,ifok,ifxj,zt,mytj,zjuid,zdxh","'".$bh."','".$_SESSION[FCWuser]."','".$sj."','".$sj."','".$uip."','出售',".$_SESSION[FCWuserID].",1,'".sqlzhuru($_POST[R1])."',0,0,99,0,'".$row1[zjuid]."',0");
 php_toheader("chushou.php?bh=".$bh);

}elseif($_GET[control]=="update"){
 zwzr();
 updatetable("fcw_fang","wylx='".sqlzhuru($_POST[R1])."' where uid='".$_SESSION[FCWuser]."' and type1='出售' and bh='".$_GET[bh]."'");
 php_toheader("chushou.php?bh=".$_GET[bh]);
...
<div class="right">
 ...
 function tj(){
 tjwait();
 f1.action="chushoulx.php?control=add";
 }
 </script>
 ...
 while0("id,bh,uid,type1,wylx","fcw_fang where uid='".$_SESSION[FCWuser]."' and id=".$_GET[id]." and type1='出售'");if(!$row=mysql_fetch_array($res)){php_toheader("chushoulist.php");}
 ?>
 <script language="javascript">
 function tj(){
 tjwait();
 f1.action="chushoulx.php?control=update&bh=<?=$row[bh]?>";
 }
 </script>
 <form name="f1" method="post" onsubmit="return tj()">
 <input type="hidden" value="chushou" name="jvs" />
 ...
 <ul class="uk">
 <li class="l3"></li>
 <li class="l4"><? tjbtnr("下一步","chushou.php?bh=".$row[bh])?></li>
 </ul>
 </form>
