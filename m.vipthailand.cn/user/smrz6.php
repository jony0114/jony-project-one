...
if(sqlzhuru($_POST[jvs])=="sf"){
 zwzr();
 if(panduan("uid,sfzrz","fcw_user where uid='".$_SESSION[FCWuser]."' and (sfzrz=2 or sfzrz=3)")==0){Audit_alert("错误的请求！","smrz6.php");}
 $sfz=sqlzhuru($_POST[tsfz]);
 if(strlen(stripos($sfz,"/"))>0 || strlen(stripos($sfz,";"))>0){Audit_alert("身份证号码格式有误！","smrz6.php");}
 ...
 php_toheader("smrz6.php"); 

}elseif(sqlzhuru($_POST[jvs])=="yy"){
 if(panduan("uid,yyzzrz","fcw_user where uid='".$_SESSION[FCWuser]."' and (yyzzrz=2 or yyzzrz=3)")==0){Audit_alert("错误的请求！","smrz6.php");}
 $yyzz=sqlzhuru($_POST[tyyzz]);
 if(strlen(stripos($yyzz,"/"))>0 || strlen(stripos($yyzz,";"))>0){Audit_alert("营业执照号码格式有误！","smrz6.php");}
 updatetable("fcw_user","
