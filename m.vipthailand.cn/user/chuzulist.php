...
if($_GET[control]=="gx"){
 ...
 if($_GET[jl]=="none"){
  while1("*","fcw_fang where type1='出租' and (uid='".$_SESSION[FCWuser]."' or zjuid='".$_SESSION[FCWuser]."') and zt=0 order by sj desc");
...
$ses=" where (uid='".$_SESSION[FCWuser]."' or zjuid='".$_SESSION[FCWuser]."') and type1='出租' and zt<>99";
...
function gxfang(x){
 ...
 if(confirm("确定要更新吗？")){location.href="chuzulist.php?control=gx&jl="+v;}else{return false;}
}
...
<div class="yjcode">
<ul class="dqwz">
<li class="l1">您的位置：<a href="../" class="acy">首页</a> > <a href="./" class="acy">会员中心</a> > 我的出租信息</li>
</ul>
<? include("left.php");?>
<!--RB-->
<div class="right">
 ...
 <li class="l2">最后更新：<?=$row[sj]?></li>
  ...
  <? if(0==$row[zt]){?>
  <? if(0==$ifzd){?>
  <a href="adzd.php?bh=<?=$row[bh]?>&ty=chuzu" class="a1">购买置顶位</a>
  ...
  <a href="../rent/view<?=$row[id]?>.html" target="_blank">预览</a>
  </li>
  </ul>
 <?
 }
