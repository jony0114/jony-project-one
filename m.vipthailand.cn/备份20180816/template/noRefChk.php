...
if($admin==1){ //表示会员登录，并判断是否有发布权限
 ...
 intotable("fcw_user","uid,pwd,usertype,sj,uip,email,money1,jf,djl,qx,sfzrz,yyzzrz,indexpm,sxnum,ifqq,zfmm","'".$uid."','".sha1($_GET[pwd])."',1,'".$sj."','".$uip."','".$_GET["mail"]."',0,0,0,'".$qx."',3,3,0,0,0,'".sha1(sqlzhuru($_GET[tpwd]))."'");$id=mysql_insert_id();
 userregc($id);
