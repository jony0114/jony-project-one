...
if($_GET[control]=="gx"){
 ...
 if($_GET[jl]=="none"){
  while1("*","fcw_fang where type1='求租' and uid='".$_SESSION[FCWuser]."' and zt=0 order by sj desc");
...
php_toheader("qiuzulist.php?t=suc");

}
//FUNE

$ses=" where (uid='".$_SESSION[FCWuser]."' or zjuid='".$_SESSION[FCWuser]."') and type1='求租' and zt<>99";
...
function ser(){
location.href="qiuzulist.php?st1="+document.getElementById("st1").value+"&sd1="+document.getElementById("sd1").value;	
}
function gxfang(x){
 if(x==1){
 ...
 if(confirm("确定要更新吗？")){location.href="qiuzulist.php?control=gx&jl="+v;}else{return false;}
}
</script>
</head>
<body>
...
<div class="yjcode">
<ul class="dqwz">
<li class="l1">您的位置：<a href="../" class="acy">首页</a> > <a href="./" class="acy">会员中心</a> > 我的求租信息</li>
</ul>
<? include("left.php");?>
<!--RB-->
<div class="right">
 ...
 <? $xs="qiuzu";while1("*","fcw_wylx where ifuse='on' and xs like '%".$xs."%' order by xh asc");while($row1=mysql_fetch_array($res1)){?>
 ...
 <li class="l2">最后更新：<?=$row[sj]?></li>
  ...
  <li class="l2"><strong class="feng"><?=returnqiu($row[money1],$row[money2],"元")?></strong></li>
  <li class="l3"><?=$row[djl]?></li>
  <li class="l4 hui">
  <? if(0==$row[zt]){?>
  <? if(0==$ifzd){?>
  <a href="adzd.php?bh=<?=$row[bh]?>&ty=qiuzu" class="a1">购买置顶位</a>
  <? }elseif(1==$ifzd){echo "<strong class='red'>已置顶</strong><br>位置:".$row[zdxh]."<br>剩".$sjcv;}?>
  <? }?>
  </li>
  <li class="l5">
  <a href="<?=$aurl?>">修改</a><br>
  <a href="../qiuzu/view<?=$row[id]?>.html" target="_blank">预览</a>
  </li>
  </ul>
 <?
 }
