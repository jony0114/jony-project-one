...
if($_GET[control]=="update"){
 zwzr();
 if(panduan("id,zjuid","fcw_user where zjuid='".$_SESSION[FCWuser]."' and id=".$_GET[id])==0){Audit_alert("权限出错","userlist.php");}
 ...
 php_toheader("user.php?t=suc&action=update&id=".$_GET[id]);

}elseif($_GET[control]=="have"){
 zwzr();
 $zh=sqlzhuru($_POST[tzh]);
 if(panduan("uid,pwd,usertype","fcw_user where uid='".$zh."' and pwd='".sha1($_POST[tmm])."' and usertype=2")==0){Audit_alert("身份核实有误，返回重试","user.php");}
 updatetable("fcw_user","zjuid='".$_SESSION[FCWuser]."' where uid='".$zh."'");
 updatetable("fcw_fang","zjuid='".$_SESSION[FCWuser]."' where uid='".$zh."'");
 updatetable("fcw_kehu","zjuid='".$_SESSION[FCWuser]."' where uid='".$zh."'");
 updatetable("fcw_kehugj","zjuid='".$_SESSION[FCWuser]."' where uid='".$zh."'");
 updatetable("fcw_hetong","zjuid='".$_SESSION[FCWuser]."' where uid='".$zh."'");
 updatetable("fcw_htcaiwu","zjuid='".$_SESSION[FCWuser]."' where uid='".$zh."'");
 php_toheader("userlist.php");
...
<div class="yjcode">
<ul class="dqwz">
<li class="l1">您的位置：<a href="../" class="acy">首页</a> > <a href="./" class="acy">会员中心</a> > 中介成员管理</li>
</ul>
<? include("left.php");?>
<!--RB-->
<div class="right">
 ...
 <? systs("恭喜您，操作成功!","user.php?id=".$_GET[id]."&action=".$_GET[action])?>
 ...
 tjwait();
 f2.action="user.php?control=have";
 }
 </script>
 <form name="f2" method="post" onsubmit="return tj1()">
 <ul class="uk">
 <li class="l1"><span class="red" style="font-weight:normal;">*</span> 成员帐号：</li>
 <li class="l2"><input type="text" autocomplete="off" class="inp" disableautocomplete name="tzh" /> 请输入<span class="red">经纪人会员性质</span>的账号</li>
 ...
 while0("*","fcw_user where zjuid='".$_SESSION[FCWuser]."' and id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("userlist.php");}
 ?>
 <script language="javascript">
 function tj(){
 ...
 tjwait();
 f1.action="user.php?control=update&id=<?=$row[id]?>";
 }
 </script>
