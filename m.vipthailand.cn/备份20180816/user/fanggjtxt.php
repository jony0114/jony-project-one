...
$mybh=$_GET[mybh];
while0("*","fcw_fanggj where (userid=".$userid." or zjuserid=".$userid.") and mybh='".$mybh."'");
if(!$row=mysql_fetch_array($res)){Audit_alert("来源错误！","./","parent.");}

if($_GET[control]=="update"){
 zwzr();
 $s1=sqlzhuru($_POST[s1]);
 if(empty($s1)){Audit_alert("跟进信息不得为空！","fanggjtxt.php?bh=".$bh."&mybh=".$mybh);}
 updatetable("fcw_fanggj","txt='".$s1."',zt=0 where id=".$row[id]);
 php_toheader("fanggjtxt.php?t=suc&bh=".$bh."&mybh=".$mybh);
}
?>
...
f1.action="fanggjtxt.php?control=update&bh=<?=$bh?>&mybh=<?=$mybh?>";
}
<? if($_GET["t"]=="suc"){?>
