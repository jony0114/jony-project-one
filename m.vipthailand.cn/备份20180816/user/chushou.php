...
sesCheck();
$bh=$_GET[bh];
while0("*","fcw_fang where (uid='".$_SESSION[FCWuser]."' or zjuid='".$_SESSION[FCWuser]."') and bh='".$bh."' and type1='出售'");
...
if($_GET[control]=="update" && $_POST[jvs]=="chushou"){
 ...
 updatetable("fcw_fang",
			 ...
			 lxid=".$lxid.",jbid=".$jbid.",
			 zt=".$zt.",
			 cq='".sqlzhuru($_POST[tcq])."',video='".sqlzhuru($_POST[tvideo])."',
 ...
 php_toheader("fangsuc.php?bh=".$bh."&cz=chushou&id=".$id);
...
<div class="yjcode">
<ul class="dqwz">
<li class="l1">您的位置：<a href="../" class="acy">首页</a> > <a href="./" class="acy">会员中心</a> > 发布出售信息</li>
</ul>
<? include("left.php");?>
<!--RB-->
<div class="right">
 <? returnuserqx($userqx,",3,");?>
 <? include("chushoucap.php");?>
 <script language="javascript">
 document.getElementById("step2").className="l1 l11";
 </script>
 <div class="rmenucap">
 <a href="chushou.php?bh=<?=$bh?>" class="a1">房源信息</a>
 ...
 <li class="l1">售价：</li>
 <li class="l2"><input type="text" class="inp" name="tmoney1" size="5" value="<?=returnzdv($row[money1])?>" /><span class="fd">万 (小提示：不填表示面议)</span></li>
 ...
 <li class="l1">层数：</li>
 <li class="l2"><input name="tlc1" value="<?=returnzdv($row[lc1])?>" style="width:26px;" class="inp" type="text" />
 <span class="fd">层 (小提示：如不清楚，可以不用填写层数)</span></li>
 <? }?>
 
 <? if(check_in($wylx,"住宅,公寓,写字楼,商铺,旅馆/酒店")){?>
 <li class="l1">楼层：</li>
 <li class="l2">
 <span class="fd">第</span><input name="tlc1" value="<?=returnzdv($row[lc1])?>" class="inp" style="width:26px;margin-left:7px;" type="text" />
 <span class="fd">层，共</span> 
 ...
 <div class="rts" style="cursor:pointer;" onclick="xtinfonc()">【点击<span id="xtzs">收起</span>】 选填部分可以不填写，但完善选填信息有助于您的房源更快被出售，因此我们也建议您能耐心完善好。</div>
 <div id="xuantian">
 <ul class="uk">
 ...
 <li class="l1">物业费：</li>
 <li class="l2"><input name="tmoney2" value="<?=returnzdv($row[money2])?>" style="width:120px;" class="inp" type="text" /><span class="fd">(小提示：如不清楚或没有物业费，可以留空)</span></li>
 <? }?>
 </ul>
 ...
 <? $i=1;while1("*","fcw_school where admin=1 order by xh asc");while($row1=mysql_fetch_array($res1)){?>
 <ul class="uk1 uk10" id="schoolm<?=$i?>" style="display:none;">
 ...
 <span class="red">支持腾讯视频和优酷视频，复制播放地址即可</span>
 </li>
