<?
//修改该文件要同步修改fcwmanage/qiuzulx.php以及手机端
include("../config/conn.php");
include("../config/function.php");
include("../config/tpclass.php");
sesCheck();
$bh=$_GET[bh];
while0("*","fcw_fang where (uid='".$_SESSION[FCWuser]."' or zjuid='".$_SESSION[FCWuser]."') and bh='".$bh."' and type1='求租'");
if(!$row=mysql_fetch_array($res)){php_toheader("qiuzulist.php");}
$wylx=$row[wylx];
$muid=$row[uid];
$id=$row[id];
...
//B
if($_GET[control]=="update" && $_POST[jvs]=="qiuzu"){
 zwzr();
 $sj=date("Y-m-d H:i:s");
 ...
 php_toheader("fangsuc.php?bh=".$bh."&cz=qiuzu&id=".$id);
...
function tj(){
 if((document.f1.ttit.value).replace(/\s/,"")==""){alert("请输入求租标题！");document.f1.ttit.focus();return false;}
 ...
 tjwait();
 f1.action="qiuzu.php?control=update&bh=<?=$row[bh]?>";
}
</script>
...
<div class="yjcode">
<ul class="dqwz">
<li class="l1">您的位置：<a href="../" class="acy">首页</a> > <a href="./" class="acy">会员中心</a> > 发布求租信息</li>
</ul>
<? include("left.php");?>
<!--RB-->
<div class="right">
 ...
 <? systs("恭喜您，操作成功!","qiuzu.php?bh=".$bh)?>
 <form name="f1" method="post" onsubmit="return tj()">
  <input type="hidden" value="qiuzu" name="jvs" />
 <ul class="uk">
 <li class="l1">求租类型：</li>
 <li class="l21"><strong><?=returnwylx(4,$row[wylx])?></strong>  &nbsp;&nbsp;&nbsp;[<a href="qiuzulx.php?action=update&id=<?=$row[id]?>">修改类型</a>]</li>
 ...
 <input name="tmoney2" value="<?=returnzdv($row[money2])?>" class="inp" style="width:46px;" type="text" />
 <span class="fd">
 <select name="tjgdw">
 ...
 <ul class="uk">
 <li class="l3"></li>
 <li class="l4"><? tjbtnr("下一步","qiuzulist.php")?></li>
 </ul>
 </form>
