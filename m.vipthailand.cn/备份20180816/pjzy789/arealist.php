...
<div class="right">
 ...
 <div class="bqu1">
 <a class="a1" href="arealist.php">区域管理</a>
 </div>

 <!--begin-->
 <ul class="ksedi">
 <li class="l2">
 <a href="area.php" class="a1">新增区域</a>
 <a href="javascript:void(0)" onclick="checkDEL(1,'fcw_area')" class="a2">删除</a>
 </li>
 </ul>
 <ul class="qjlistcap">
 ...
 $nu="area.php?action=update&id=".$row1[id];
 ?>
 <ul class="qjlist1">
 <li class="l1"><input name="C1" id="chk<?=$row1[id]?>" type="checkbox" value="<?=$row1[id]?>xcf0" /></li>
 <li class="l2"><a href="<?=$nu?>">[<?=$row1[py]?>] <?=$row1[name1]?></a> <? if(1==$row1[ifhot]){?><span class="red">(热)</span><? }?></li>
 <li class="l3"><?=$row1[xh]?></li>
 <li class="l4"><?=$row1[sj]?></li>
 <li class="l5">
 <a href="javascript:void(0);" onclick="del(<?=$row1[id]?>)">删除</a><span></span>
 <a class="add" href="area1.php?ty1id=<?=$row1[id]?>">添加子类</a><span></span>
 <a class="edi" href="<?=$nu?>">修改信息</a>
 </li>
 </ul>
 <?
 while2("*","fcw_area where admin=2 and name1='".$row1[name1]."' order by xh asc");while($row2=mysql_fetch_array($res2)){
 $nu="area1.php?action=update&id=".$row2[id]."&ty1id=".$row1[id]; 
 ?>
 <ul class="qjlist2">
 <li class="l1"><input name="C1" id="chk<?=$row2[id]?>" type="checkbox" value="xcf<?=$row2[id]?>" /></li>
 <li class="l2" onclick="gourl('<?=$nu?>')"> <?=$row2[py]?> - <?=$row2[name2]?> <? if(1==$row2[ifhot]){?><span class="red">(热)</span><? }?></li>
