...
if($_GET[control]=="add"){
 ...
 if(!$row1=mysql_fetch_array($res1)){Audit_alert("会员验证有误！","qiuzulx.php");}
 $bh=time()."f".$row1[id];
 $sj=date("Y-m-d H:i:s");
 $uip=$_SERVER["REMOTE_ADDR"];
 intotable("fcw_fang","bh,uid,sj,uip,type1,fbty,djl,wylx,ifok,ifxj,zt,mytj,zjuid,zdxh","'".$bh."','".sqlzhuru($_POST[tuid])."','".$sj."','".$uip."','求租',".$row1[usertype].",1,'".sqlzhuru($_POST[R1])."',0,0,99,0,'".$row1[zjuid]."',0");
 php_toheader("qiuzu.php?bh=".$bh);

}elseif($_GET[control]=="update"){
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0101,")){Audit_alert("权限不够","default.php");}
 zwzr();
 updatetable("fcw_fang","wylx='".sqlzhuru($_POST[R1])."' where type1='求租' and bh='".$_GET[bh]."'");
...
<div class="right">
 ...
 <div class="bqu1">
 <a href="javascript:void(0);" class="a1">发布求租</a>
 </div> 
 ...
 <input type="hidden" name="jvs" value="qiuzu" />
 <ul class="uk1">
 <li class="l1">我要求租：</li>
 <li class="l2">
 <? 
 ...
 while0("id,bh,uid,type1,wylx","fcw_fang where id=".$_GET[id]." and type1='求租'");if(!$row=mysql_fetch_array($res)){php_toheader("qiuzulist.php");}
 ?>
 <script language="javascript">
 function tj(){
 layer.msg('正在提交', {icon: 16  ,time: 0,shade :0.25});
