...
if($_GET[control]=="update"){
 ...
 if(empty($company) || empty($uadd) || empty($nc)){Audit_alert("错误的路径来源！","user4.php");}
 ...
 updatetable("fcw_user","areaid=".sqlzhuru($_POST[area1]).",areaids=".sqlzhuru($_POST[area2]).",uadd='".$uadd."' where zjuid='".sqlzhuru($_POST[tuid])."'");
 php_toheader("user4.php?t=suc&id=".$id);
...
function tj(){
 ...
 f1.action="user4.php?control=update&id=<?=$id?>&qx="+str+"&ife="+ife+"&ifm="+ifm;
}
</script>
</head>
<body>
...
<div class="right">
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！","user4.php?id=".$id);}?>
 ...
 <li class="l1"><span class="red">*</span> 门店地址：</li>
 <li class="l2"><input type="text" size="50" value="<?=$row[uadd]?>" class="inp" name="tuadd" /></li>
 <li class="l1"><span class="red">*</span> 中介名称：</li>
 ...
 <li class="l2"><input  name="tseokey" value="<?=$row[seokey]?>" size="60" type="text" class="inp" onfocus="inpf(this)" onblur="inpb(this)" /></li>
 <li class="l4">店铺描述：</li>
 ...
 <label><input name="C1" type="checkbox" value="7"<? if(strstr($row[qx],",7,")){?> checked="checked"<? }?>/> 管理员工</label> 
 </li>
 <li class="l1">推广排序：</li>
