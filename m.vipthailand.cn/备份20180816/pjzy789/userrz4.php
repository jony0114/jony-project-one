...
if(sqlzhuru($_POST[jvs])=="sf"){
 ...
 if(strlen(stripos($sfz,"/"))>0 || strlen(stripos($sfz,";"))>0){Audit_alert("身份证号码格式有误！","userrz4.php?id=".$id);}
 ...
 php_toheader("userrz4.php?t=suc&id=".$id); 

}elseif(sqlzhuru($_POST[jvs])=="yy"){
 $yyzz=sqlzhuru($_POST[tyyzz]);
 if(strlen(stripos($yyzz,"/"))>0 || strlen(stripos($yyzz,";"))>0){Audit_alert("营业执照号码格式有误！","userrz4.php?id=".$id);}
 updatetable("fcw_user","
...
<div class="right">
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！","userrz4.php?id=".$id);}?>
