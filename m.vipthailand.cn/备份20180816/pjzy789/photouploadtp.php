...
while1("*","fcw_jia_photo where bh='".$bh."'");if(!$row1=mysql_fetch_array($res1)){echo "1";exit;}
...
if (!empty($_FILES) && sqlzhuru($_POST['token']) == $verifyToken) {
	...
	if (in_array(strtolower($fileParts['extension']),$fileTypes)) {
		...
		intotable("fcw_tp","bh,tp,type1,iffm,sj,uid,xh","'".$bh."','".$wjv."','×°ÐÞÐ§¹ûÍ¼',0,'".$sj."','".$row1[uid]."',".$nxh."");
		echo '1';
	} else {
