...
if($_GET[control]=="add"){
 ...
 zwzr();
 if(panduan("*","fcw_sphy where admin=2 and name1='".sqlzhuru($_POST[t0])."' and name2='".sqlzhuru($_POST[t1])."'")==1){Audit_alert("该行业已存在！","sphy1.php?ty1id=".$_GET[ty1id]);}
 ...
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){Audit_alert("权限不够","default.php");}
 zwzr();
 if(panduan("*","fcw_sphy where admin=2 and name1='".sqlzhuru($_POST[t0])."' and name2='".sqlzhuru($_POST[t1])."' and id<>".$_GET[id])==1){Audit_alert("该行业已存在！","sphy1.php?action=update&id=".$_GET[id]."&ty1id=".$_GET[ty1id]);}
...
<div class="right">
 ...
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！","sphy1.php?action=".$_GET[action]."&ty1id=".$_GET[ty1id]."&id=".$_GET[id]);}?>

 <div class="bqu1">
 <a href="javascript:void(0);" class="a1">行业信息</a>
 <a href="sphylist.php">返回列表</a>
 </div> 
 ...
 <script language="javascript">
 function tj(){
 if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入行业名称！");document.f1.t1.focus();return false;}
 ...
 <li class="l1">排序：</li>
 <li class="l2"><input type="text" class="inp" name="t2" onfocus="inpf(this)" onblur="inpb(this)" value="<?=returnxh("fcw_sphy"," and admin=2 and name1='".$row[name1]."'")?>" /><span class="fd">序号越小，越靠前</span></li>
