...
if($_GET[control]=="update"){
 ...
 if(empty($nc)){Audit_alert("错误的路径来源！","user7.php");}
 if(empty($row[zjuid])){$ses="areaid=".sqlzhuru($_POST[area1]).",areaids=".sqlzhuru($_POST[area2]).",uadd='".sqlzhuru($_POST[tuadd])."',company='".sqlzhuru($_POST[tcompany])."',";}
 $pwd=sqlzhuru($_POST[tpwd]);if(!empty($pwd)){$ses=$ses."pwd='".sha1($pwd)."',";}
 ...
 php_toheader("user7.php?t=suc&id=".$id);
...
function tj(){
 ...
 f1.action="user7.php?control=update&id=<?=$id?>&qx="+str+"&ife="+ife+"&ifm="+ifm;
}
</script>
</head>
<body>
...
<div class="right">
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！","user7.php?id=".$id);}?>
 ...
 <li class="l1">公司地址：</li>
 <li class="l2"><input type="text" class="inp" size="50" name="tuadd" value="<?=$row[uadd]?>" /></li>
 <li class="l1">公司名称：</li>
 <li class="l2"><input type="text" class="inp" name="tcompany" value="<?=$row[company]?>" /></li>

 <? 
 }else{
 while1("uid,areaid,areaids,company,uadd,zjuid","fcw_user where uid='".$row[zjuid]."'");$row1=mysql_fetch_array($res1);
 ?>
 <li class="l1">所在公司：</li>
 ...
 <? }?>
 <li class="l1">个人头像：</li>
 <li class="l2"><input type="file" name="inp1" class="inp1" id="inp1" size="15" accept=".jpg,.gif,.jpeg,.png"></li>
 <li class="l8"></li>
 <li class="l9"><img src="../upload/<?=$id?>/user.jpg?t=<?=rnd_num(100)?>" width="54" height="54" /></li>
 <li class="l10">自我介绍：</li>
 ...
 <label><input name="C1" type="checkbox" value="1"<? if(strstr($row[qx],",1,")){?> checked="checked"<? }?>/> 装修案例管理</label>
 <label><input name="C1" type="checkbox" value="6"<? if(strstr($row[qx],",6,")){?> checked="checked"<? }?>/> 独立商铺</label>
 </li>
