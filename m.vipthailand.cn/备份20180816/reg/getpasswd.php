...
if(sqlzhuru($_POST[jvs])=="getpwd"){
 ...
 $m=sqlzhuru($_POST[t1]);
 if(strtolower($_SESSION["NYZSES"])!=strtolower(sqlzhuru($_POST[t2]))){Audit_alert("验证码有误！","getpasswd.php");}
 while0("id,uid,email","fcw_user where email='".$m."' and uid='".$uid."'");if(!$row=mysql_fetch_array($res)){Audit_alert("帐号与邮箱不匹配！","getpasswd.php");}
