...
if($_GET[control]=="add"){
 ...
 zwzr();
 if(panduan("*","fcw_sphy where admin=1 and name1='".sqlzhuru($_POST[t1])."'")==1){Audit_alert("该行业已存在！","sphy.php");}
 ...
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){Audit_alert("权限不够","default.php");}
 zwzr();
 if(panduan("*","fcw_sphy where admin=1 and name1='".sqlzhuru($_POST[t1])."' and id<>".$_GET[id])==1){Audit_alert("该行业已存在！","sphy.php?action=update&id=".$_GET[id]);}
...
<div class="right">
 ...
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！","sphy.php?action=".$_GET[action]."&id=".$_GET[id]);}?>
 ...
 <li class="l1">排序：</li>
 <li class="l2"><input type="text" class="inp" name="t2" value="<?=returnxh("fcw_sphy"," and admin=1")?>" /><span class="fd">序号越小，越靠前</span></li>
