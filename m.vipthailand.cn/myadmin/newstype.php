...
if($_GET[control]=="add"){
 ...
 zwzr();
 if(panduan("*","fcw_newstype where admin=1 and name1='".sqlzhuru($_POST[t1])."'")==1){Audit_alert("该资讯分类已存在！","newstype.php");}
 intotable("fcw_newstype","name1,sj,xh,admin,xswz,xstype","'".sqlzhuru($_POST[t1])."','".$sj."',".sqlzhuru($_POST[t2]).",1,'".$_GET[xs]."','".sqlzhuru($_POST[R1])."'");
 php_toheader("newstype.php?t=suc");
 
}elseif($_GET[control]=="update"){
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){Audit_alert("权限不够","default.php");}
 zwzr();
 if(panduan("*","fcw_newstype where admin=1 and name1='".sqlzhuru($_POST[t1])."' and id<>".$_GET[id])==1){Audit_alert("该资讯分类已存在！","newstype.php?action=update&id=".$_GET[id]);}
 updatetable("fcw_newstype","name1='".sqlzhuru($_POST[t1])."' where name1='".sqlzhuru($_POST[oldty1])."'");
 updatetable("fcw_newstype","sj='".$sj."',xh=".sqlzhuru($_POST[t2]).",xstype='".sqlzhuru($_POST[R1])."',xswz='".$_GET[xs]."' where id=".$_GET[id]);
...
<div class="right">
 ...
 <li class="l1">排序：</li>
 <li class="l2"><input type="text" class="inp" name="t2" value="<?=returnxh("fcw_newstype"," and admin=1")?>" /> <span class="fd">序号越小，越靠前</span></li>
 <li class="l1">显示方式：</li>
 <li class="l2">
 ...
 while0("*","fcw_newstype where id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("newstypelist.php");}
 ?>
 <script language="javascript">
 function tj(){
 if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入名称！");document.f1.t1.focus();return false;}
 ...
 <li class="l1">显示方式：</li>
 <li class="l2">
 <label><input name="R1" type="radio" value="font" <? if($row[xstype]=="font"){?> checked="checked"<? }?> /> 文字列表</label>
 <label><input name="R1" type="radio" value="pic" <? if($row[xstype]=="pic"){?> checked="checked"<? }?> /> 图片列表</label>
 </li>
 <li class="l3"><input type="submit" value="保存修改" class="btn1" /></li>
 </ul>
 </form>
