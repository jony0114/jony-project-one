...
<div class="right">
 ...
 ?>
 <a <? if($tpnumarr[$tpnum]==$tpnumarr[$i]){echo "class='a1'";}?> href="xqphotolist.php?bh=<?=$bh?>&tpnum=<?=$i?>"><?=$tptyarr[$i]?>(<?=returncount("fcw_xqphoto where uid='".$uid."' and xqbh='".$bh."' and zt<>99 and tyid=".$tpnumarr[$i])?>)</a>
 <? }?>
 </div>
 ...
 <?
 pagef($ses,20,"fcw_xqphoto","order by sj desc");while($row=mysql_fetch_array($res)){
 ...
 <li class="l3"><?=$tptyarr[$row[tyid]]?></li>
