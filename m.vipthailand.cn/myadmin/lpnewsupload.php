...
AdminSes_audit();
if(!strstr($adminqx,",0,") && !strstr($adminqx,",0401,")){echo "无权限";exit;}
$bh=$_GET[bh];
while1("bh,uid,xqbh,sj","fcw_xqnews where bh='".$bh."'");if(!$row1=mysql_fetch_array($res1)){exit;}
$userid=returnuserid($row1[uid]);
$uid=$row1[uid];
$tplj1="upload/".$userid."/f/".$row1[xqbh]."/".$bh."/";

 if($_GET[action]=="update"){  //上传
 $sj=date("Y-m-d H:i:s");
 $nbh=time();
 $wjv=$tplj1.$nbh.".jpg";
 intotable("fcw_clear","bh,tp,type1,sj","'".$_GET[bh]."','".$wjv."','".$_GET[ty]."','".$sj."'");$clearid=mysql_insert_id();
 uploadtpnodata(1,$tplj1,$nbh.".jpg","allpic",600,600,120,90);
 php_toheader("lpnewsupload.php?bh=".$bh."&tpbh=".$nbh."&clearid=".$clearid);
 
 }elseif($_GET[action]=="del"){ //删除
  if(panduan("uid,bh","fcw_xqnews where uid='".$uid."' and bh='".$_GET[bh]."'")==1){
  ...
  php_toheader("lpnewsupload.php?bh=".$_GET[bh]);
  }
 }
...
document.getElementById("uping").style.display="";
location.href="lpnewsupload.php?tpbh=<?=$_GET[tpbh]?>&action=del&bh=<?=$_GET[bh]?>&tpid=<?=$_GET[tpid]?>&clearid=<?=$_GET[clearid]?>&ty=<?=urlencode($_GET[ty])?>";	
}
</script>
</head>
<body>
...
<!--等待上传开始-->
<form method="post" name="tpf" enctype="multipart/form-data" action="lpnewsupload.php?tpbh=<?=$_GET[tpbh]?>&action=update&bh=<?=$_GET[bh]?>&ty=<?=urlencode($_GET[ty])?>">
