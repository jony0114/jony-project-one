...
if($_GET[control]=="add"){
 ...
 zwzr();
 if(panduan("*","fcw_peitao where name1='".sqlzhuru($_POST[t1])."'")==1){Audit_alert("该配套名称已存在！","wypt.php");}
 intotable("fcw_peitao","name1,sj,xh,cswy,czwy","'".sqlzhuru($_POST[t1])."','".$sj."',".sqlzhuru($_POST[t2]).",'".$_GET[cs]."','".$_GET[cz]."'");
 php_toheader("wypt.php?t=suc");
 
}elseif($_GET[control]=="update"){
 if(!strstr($adminqx,",0,") && !strstr($adminqx,",0301,")){Audit_alert("权限不够","default.php");}
 zwzr();
 if(panduan("*","fcw_peitao where name1='".sqlzhuru($_POST[t1])."' and id<>".$_GET[id])==1){Audit_alert("该配套名称已存在！","wypt.php?action=update&id=".$_GET[id]);}
 updatetable("fcw_peitao","name1='".sqlzhuru($_POST[t1])."',sj='".$sj."',xh=".sqlzhuru($_POST[t2]).",cswy='".$_GET[cs]."',czwy='".$_GET[cz]."' where id=".$_GET[id]);
 php_toheader("wypt.php?t=suc&action=update&id=".$_GET[id]);
...
<div class="right">
 ...
 <? if($_GET[t]=="suc"){systs("恭喜您，操作成功！","wypt.php?action=".$_GET[action]."&id=".$_GET[id]);}?>
 
 <div class="bqu1">
 <a href="javascript:void(0);" class="a1">物业配套</a>
 <a href="wyptlist.php">返回列表</a>
 </div> 
 ...
 f1.action="wypt.php?control=add&cs="+str+"&cz="+str1;
 }
 </script>
 ...
 <ul class="uk uk0">
 <li class="l1">排序：</li>
 <li class="l2"><input type="text" class="inp" name="t2" value="<?=returnxh("fcw_peitao","")?>" /> <span class="fd">序号越小，越靠前</span></li>
 ...
 while0("*","fcw_peitao where id=".$_GET[id]);if(!$row=mysql_fetch_array($res)){php_toheader("wyptlist.php");}
 ?>
 <script language="javascript">
 function tj(){
 if((document.f1.t1.value).replace(/\s/,"")==""){alert("请输入名称！");document.f1.t1.focus();return false;}
 ...
 layer.msg('正在提交', {icon: 16  ,time: 0,shade :0.25});
 f1.action="wypt.php?control=update&id=<?=$row[id]?>&cs="+str+"&cz="+str1;
 }
 </script>
 <form name="f1" method="post" onsubmit="return tj()">
 <ul class="uk">
